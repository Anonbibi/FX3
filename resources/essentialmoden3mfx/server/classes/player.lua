...
function CreatePlayer(source, permission_level, money, bank, identifier, group, dirtymoney, nom, prenom, phoneNumber, isFirstConnection, jobId, jobName, jobSalary)
	local self = {}
...
------------ SETTER --------
	rTable.setName = function(name)
		self.nom = name
	end

	rTable.setLastName = function(lastname)
		self.prenom = lastname
	end

	rTable.setPhoneNumber = function(phone_number)
		self.phone_number = phone_number
	end

	rTable.setJobId = function(job_id)
		self.job_id = job_id
		MySQL.Async.fetchScalar("SELECT * FROM jobs WHERE job_id = '@job_id'", {['@job_id'] = job_id}, function (result)
