...
AddEventHandler('hcdh:receiveIsMembre', function(result)
	Citizen.Trace('isMembreisMembreisMembreisMembreisMembreisMembreisMembre')
...
AddEventHandler('hcdh:forcedEnteringVeh', function(veh)
	if(handCuffed) then
...
function getIsInServiceHcdh()
	return isMembreInService
end
...
function ServiceOn()
	isMembreInService = true
	--TriggerServerEvent("jobssystem:jobs", 2)
	TriggerServerEvent("hcdh:takeService")
end

function ServiceOff()
	isMembreInService = false
	--TriggerServerEvent("jobssystem:jobs", 7)
	TriggerServerEvent("hcdh:breakService")
	allServiceMembres = {}
        ...
        if(isMembre) then
			...
			if(isMembreInService) then
				if(isNearStationGarage()) then
					if(hcdhvehicle ~= nil) then --existingVeh
						DisplayHelpText('Appuyer sur ~INPUT_CONTEXT~ pour ranger votre vehicule',0,1,0.5,0.8,0.6,255,255,255,255)
					else
						DisplayHelpText('Appuyer sur ~INPUT_CONTEXT~ pour ouvrir le garage de hcdh',0,1,0.5,0.8,0.6,255,255,255,255)
					end
					
					if IsControlJustPressed(1,51) then
						if(hcdhvehicle ~= nil) then
							SetEntityAsMissionEntity(hcdhvehicle, true, true)
							Citizen.InvokeNative(0xEA386986E786A54F, Citizen.PointerValueIntInitialized(hcdhvehicle))
							hcdhvehicle = nil
						else
							--OpenVeh()
							MenuChoixHcdhVehicleCar()
						end
					end
				end
