  ...
  TriggerEvent('es:getPlayerFromId', source, function(user)
    ...
    if (tonumber(user.money) >= tonumber(price)) then
      ...
      elseif mode == "MySQL" then
    	...
    	TriggerClientEvent("es_freeroam:notify", source, "CHAR_PROPERTY_BAR_FES", 1, "Gestion Propriéte", false, txt[lang]['welcome'])
    	TriggerClientEvent('apart:isMine', source)
    else
    	TriggerClientEvent("es_freeroam:notify", source, "CHAR_PROPERTY_BAR_FES", 1, "Gestion Propriéte", false, txt[lang]['nocash'] price)
    end
  end)
end)
...
AddEventHandler('apart:sellAppart', function(name, price)
  TriggerEvent('es:getPlayerFromId', source, function(user)
    ...
    user:addMoney((price))
      ...
      TriggerClientEvent("es_freeroam:notify", source, "CHAR_PROPERTY_BAR_FES", 1, "Gestion Propriéte", false, txt[lang]['estVendu'])
      TriggerClientEvent('apart:isNotBuy', source)
  end)
end)
