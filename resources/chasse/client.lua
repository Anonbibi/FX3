...
RegisterNetEvent('chasse:drawSellViande')
AddEventHandler('chasse:drawSellViande', function (qte)
		if(qte == nil) then
			qte = 0
		end
		if qte > 0 then
			TriggerEvent('player:looseItem',23, 1)
			local salaire = math.random(140, 180)
...
Citizen.CreateThread(function()
		...
		TriggerEvent('chasse:createBlip2', 141, 994.641, -2158.57, 29.4765)
       ...
       playerPed = GetPlayerPed(-1)
		...
		if pos then
				...
				if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 994.641,-2158.57,29.4765, true) <= 5 then
					TriggerServerEvent('chasse:serverRequest', "SellViande")
					Citizen.Wait(4000)
				end
				if (GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -1492.2, 4971.5, 64.2, true) <= 8 ) then
					...
					drawTxt("Press 'E' pour commencer à chasser ou pour ARRETER",0,1,0.5,0.8,0.6,255,255,255,255)
					if (IsControlPressed(1, 51)) then
						...
						if isinjob==false then
							...
							if not DoesEntityExist(vehiculeDetected) then
								...
								entitySpawned = false
								remover = true
								GiveWeaponToPed(GetPlayerPed(-1), GetHashKey("WEAPON_MUSKET"),30, true, true)
