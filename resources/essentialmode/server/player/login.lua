...
function LoadUser(identifier, source, new)
	MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @name", {['@name'] = identifier}, function (result)
		Users[source] = CreatePlayer(source, result[1].permission_level, result[1].money, result[1].bankbalance, result[1].identifier, result[1].group, result[1].dirty_money)
