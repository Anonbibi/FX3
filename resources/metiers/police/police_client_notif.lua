...
function callPolice(type)
    ...
    local myName = GetPlayerName(myPed)
    TriggerServerEvent('police:Call', myCoord.x, myCoord.y, myCoord.z, myName ,type)
end
...
function POLICE_Saisir_AS()
    t, distance = GetClosestPlayer()
    if(distance ~= -1 and distance < 3) then
        TriggerServerEvent("police:targetSaisirDyrtiMoney", GetPlayerServerId(t))
    else
        TriggerEvent('chatMessage', 'GOVERNMENT', {255, 0, 0}, "pas de joueur proche!")
    end
end

function POLICE_Saisir_ITEM()
    t, distance = GetClosestPlayer()
    if(distance ~= -1 and distance < 3) then
        TriggerServerEvent("police:targetSaisirIllegal", GetPlayerServerId(t))
    else
        TriggerEvent('chatMessage', 'GOVERNMENT', {255, 0, 0}, "pas de joueur proche!")
    end
end

function POLICE_Saisir_ALLWEAPONS()
    t, distance = GetClosestPlayer()
    if(distance ~= -1 and distance < 3) then
        
        TriggerServerEvent("police:targetSaisirAllWeapon", GetPlayerServerId(t))
...
function POLICE_Saisir_NOACHAWEAPONS()
    t, distance = GetClosestPlayer()
    if(distance ~= -1 and distance < 3) then
        RemoveAllPedWeapons(GetPlayerPed(t), true)
        TriggerServerEvent("police:targetSaisirNoAchaWeapon", GetPlayerServerId(t))
    else
        TriggerEvent('chatMessage', 'GOVERNMENT', {255, 0, 0}, "pas de joueur proche!")
    end
end

function POLICE_Saisir_TEMPSWEAPONS()
    t, distance = GetClosestPlayer()
    if(distance ~= -1 and distance < 3) then
        RemoveAllPedWeapons(GetPlayerPed(t), true)
    else
...
function POLICE_Rendre_WEAPONS()
    t, distance = GetClosestPlayer()
    if(distance ~= -1 and distance < 3) then
        TriggerServerEvent("police:targetSaisirRendreWeapon", GetPlayerServerId(t))
    else
        TriggerEvent('chatMessage', 'GOVERNMENT', {255, 0, 0}, "pas de joueur proche!")
    end
end
...
function POLICE_PRISON_DATA(data)
    t, distance = GetClosestPlayer()
    if(distance ~= -1 and distance < 3) then
