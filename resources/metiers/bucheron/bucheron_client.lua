...
function bucheron_InitMenuRemorque()
	MenuTitle = "SpawnJobs"
	ClearMenu()
	Menu.addButton("Remorque", "bucheron_callSE", 'bucheron:Remorque')
end
	...
	Citizen.CreateThread(function () 
		while bucheron_markerBool == true do
			...
			if isInServiceBucheron then
				...
				if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), bucheron_blips["Remorque"].x, bucheron_blips["Remorque"].y, bucheron_blips["Remorque"].z, true) <= bucheron_blips["Remorque"].distanceMarker+5 then
						DrawMarker(1, bucheron_blips["Remorque"].x, bucheron_blips["Remorque"].y, bucheron_blips["Remorque"].z, 0, 0, 0, 0, 0, 0, 2.001, 2.0001, 0.5001, 0, 155, 255, 200, 0, 0, 0, 0)
						ClearPrints()
						SetTextEntry_2("STRING")
						AddTextComponentString("Appuyez sur ~g~E~s~ pour faire apparaître/ranger votre ~b~remorque")
						DrawSubtitleTimed(2000, 1)
						if IsControlJustPressed(1, Keys["E"]) then
							if(existingRem ~= nil) then
								SetEntityAsMissionEntity(existingRem, true, true)
								Citizen.InvokeNative(0xEA386986E786A54F, Citizen.PointerValueIntInitialized(existingRem))
								existingRem = nil
							else
								bucheron_InitMenuRemorque()
								Menu.hidden = not Menu.hidden
							end
						end
				end
...
AddEventHandler('bucheron:getCar', function (source)
	...
	if not DoesEntityExist(vehiculeDetected) then
		...
		SetVehicleEnginePowerMultiplier(existingVeh, 35.0)
...
RegisterNetEvent('bucheron:getRemorque')
AddEventHandler('bucheron:getRemorque', function (source)
	local vehiculeDetected = GetClosestVehicle(bucheron_remorque.x, bucheron_remorque.y, bucheron_remorque.z, 6.0, 0, 70)
	if not DoesEntityExist(vehiculeDetected) then
		local myPed = GetPlayerPed(-1)
		local player = PlayerId()
		...
		local plate = math.random(100, 900)
		existingRem = CreateVehicle(remorque,bucheron_remorque.x, bucheron_remorque.y, bucheron_remorque.z,0.0, true, false)
		--SetVehicleEnginePowerMultiplier(existingRem, 40.0)
		--SetVehicleHasBeenOwnedByPlayer(existingRem,true)
		local id = NetworkGetNetworkIdFromEntity(existingRem)
		SetNetworkIdCanMigrate(id, true)
		SetEntityInvincible(existingRem, false)
		SetVehicleOnGroundProperly(existingRem)
		SetVehicleNumberPlateText(existingRem, bucheron_platesuffix.." "..plate.." ")
		SetModelAsNoLongerNeeded(remorque)
		Citizen.InvokeNative(0xB736A491E64A32CF, Citizen.PointerValueIntInitialized(existingRem))
	else
		notif("Zone encombrée.")
	end
end)
...
AddEventHandler('bucheron:drawSellPlanche', function (qte)
	...
	if qte > 0 then
		...
		TriggerServerEvent("coffrebuch:amendecoffre", salaire)
		ClearPrints()
		SetTextEntry_2("STRING")
