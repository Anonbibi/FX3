	...
	metiersList = {
		...
		{name="mecano", id=16},
		{name="taxi", id=17},
		{name="[LSPD] Brigadier", id=20},
		{name="[LSPD] Brigadier-Chef", id=29},
		{name="[LSPD] Sergent", id=21},
		{name="[LSPD] Sergent-Chef", id=30},
		{name="[LSPD] Lieutenant", id=22},
		{name="[LSPD] Lieutenant-Chef", id=31},
		{name="[LSPD] Capitaine", id=23},
		{name="[LSPD] Commandant", id=32},
		{name="[GOUV] Agent", id=24},
		{name="[GOUV] President", id=25},
		{name="[HCDH] Representente", id=26},
		{name="[BOSS] Metal Co", id=27},
		{name="[$] Grand Bandit", id=28},
		{name="[BOSS] LBDL", id=33},
		{name="[BOSS] TequiLaLa", id=34},
		{name="[BOSS] Garage Carreno", id=35},
		{name="[BOSS] Eco Taxi", id=36},
		{name="[ASSO] Evenews", id=37},
	...
	function MineGetPlayerJobName(JobId)
		local requestPlayerJobName = MySQL:executeQuery("SELECT * FROM jobs WHERE job_id = '@namejob'", {['@namejob'] = JobId})
	...
	AddEventHandler('es:playerLoaded', function(source)
		TriggerEvent('es:getPlayerFromId', source, function(user)
			...
			for _, item in pairs(metiersList) do
				...
				if playerJobID == 27 and item.id == 9 then
					TriggerClientEvent(item.name..":drawBlips", source)
					TriggerClientEvent(item.name..":drawMarker", source, true)
					TriggerClientEvent(item.name..":marker", source)
				end

				if playerJobID == 33 and item.id == 7 then
					TriggerClientEvent(item.name..":drawBlips", source)
					TriggerClientEvent(item.name..":drawMarker", source, true)
					TriggerClientEvent(item.name..":marker", source)
				end
