	...
	TriggerEvent('es:getPlayerFromId', source, function(user)
		...
		if (tonumber(user.money) >= tonumber(price)) then
			...
			if (tonumber(max_number_weapons) > tonumber(nb_weapon)) then
				...
				TriggerClientEvent("es_freeroam:notify", source, "CHAR_MP_ROBERTO", 1, "Gangsta", false, "Amuses-toi bien\n")
			else
				TriggerClientEvent('ToManyWeapons',source)
				TriggerClientEvent("es_freeroam:notify", source, "CHAR_MP_ROBERTO", 1, "Gangsta", false, "Vous avez atteint le nombre maxi d'armes (max: "..max_number_weapons..")\n")
			end
		else
...
AddEventHandler("weaponshop:RemoveWeaponsToPlayer", function(player)
	TriggerEvent('es:getPlayerFromId', player, function(user)
		local playerID = user.identifier			
