    ...
    while true do
      ...
      if(IsNearBank() or IsNearATM()) then
        if (atBank == false) then
          Citizen.CreateThread(function()
            while true do
              Wait(0)
              if (IsNearBank()) then
                drawTxt('Appuyer sur ~g~E~s~ pour accéder à la banque',0,1,0.5,0.8,0.6,255,255,255,255)
              elseif (IsNearATM()) then
                drawTxt('Appuyer sur ~g~E~s~ pour accéder à l\'ATM',0,1,0.5,0.8,0.6,255,255,255,255)
              end
            end
          end)
        end
        ...
        if IsControlJustPressed(1, 38)  then -- IF INPUT_PICKUP Is pressed
          if (IsInVehicle()) then
            TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Vous devez sortir de votre véhicule!");
          else
            if bankOpen then
              closeGui()
              bankOpen = false
...
AddEventHandler('bank:deposit', function(amount)
  if(IsNearBank() == true or depositAtATM == true and IsNearATM() == true or depositAnywhere == true ) then
    ...
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Vous n'êtes pas à la banque!");
  end
end)
...
AddEventHandler('bank:withdraw', function(amount)
  if(IsNearATM() == true or IsNearBank() == true or withdrawAnywhere == true) then
    ...
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Vous devez être dans une banque ou devant un ATM!");
  end
end)
...
AddEventHandler('bank:givecash', function(toPlayer, amount)
  if(IsNearPlayer(toPlayer) == true or giveCashAnywhere == true) then
    ...
    if (playing ~= false) then
      ...
      TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Ce joueur n'est pas en ligne");
    end
  else
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Tu n'es pas assez proche de ce joueur");
  end
end)
...
AddEventHandler('bank:givedirty', function(toPlayer, amount)
  if(IsNearPlayer(toPlayer) == true or giveCashAnywhere == true) then
    ...
    local playing = IsPlayerPlaying(player2)
    if (playing ~= false) then
      TriggerServerEvent("bank:givedirty", toPlayer, tonumber(amount))
    else
      TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Ce joueur n'est pas en ligne");
    end
  else
