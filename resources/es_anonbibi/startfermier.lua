...
local startfermier_blips ={}
local inrangeofstartfermier = false
...
--
local blipsnum =  85
local blipstext = 'Dépot des fermiers'
...
AddEventHandler('playerSpawned', function(spawn)
	if firstspawn == 0 then
		ShowStartfermierBlips(true)
		firstspawn = 1
	end
end)

function IsPlayerInRangeOfStartfermier()
	return inrangeofstartfermier
end

function ShowStartfermierBlips(bool)
	if bool and #startfermier_blips == 0 then
		for station,pos in pairs(startfermier_locations) do
			local loc = pos
			pos = pos.entering
			...
			table.insert(startfermier_blips, {blip = blip, pos = loc})
		end
		Citizen.CreateThread(function()
			while #startfermier_blips > 0 do
				Citizen.Wait(0)
				local inrange = false
				for i,b in ipairs(startfermier_blips) do
				...
				inrangeofstartfermier = inrange
				if IsControlJustPressed(1,controllkeyvalid)  and (inrangeofstartfermier == true) then
					...
					SetVehicleNumberPlateText(spawned_car, "FERMI")
	...
	elseif bool == false and #startfermier_blips > 0 then
		for i,b in ipairs(startfermier_blips) do
