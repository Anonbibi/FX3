...
local startbucheron_blips ={}
local inrangeofstartbucheron = false
...
--
local blipsnum =  85
local blipstext = 'Dépot des bucheron'
...
AddEventHandler('playerSpawned', function(spawn)
	if firstspawn == 0 then
		ShowStartbucheronBlips(true)
		firstspawn = 1
	end
end)

function IsPlayerInRangeOfStartbucheron()
	return inrangeofstartbucheron
end

function ShowStartbucheronBlips(bool)
	if bool and #startbucheron_blips == 0 then
		for station,pos in pairs(startbucheron_locations) do
			local loc = pos
			pos = pos.entering
			...
			table.insert(startbucheron_blips, {blip = blip, pos = loc})
		end
		Citizen.CreateThread(function()
			while #startbucheron_blips > 0 do
				Citizen.Wait(0)
				local inrange = false
				for i,b in ipairs(startbucheron_blips) do
				...
				inrangeofstartbucheron = inrange
				if IsControlJustPressed(1,controllkeyvalid)  and (inrangeofstartbucheron == true) then
					...
					SetVehicleNumberPlateText(spawned_car, "BUCHE")
	...
	elseif bool == false and #startbucheron_blips > 0 then
		for i,b in ipairs(startbucheron_blips) do
