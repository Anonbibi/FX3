...
local startvignoble_blips ={}
local inrangeofstartvignoble = false
...
--
local blipsnum =  85
local blipstext = 'Dépot des vignerons'
...
AddEventHandler('playerSpawned', function(spawn)
	if firstspawn == 0 then
		ShowStartvignobleBlips(true)
		firstspawn = 1
	end
end)

function IsPlayerInRangeOfStartvignoble()
	return inrangeofstartvignoble
end

function ShowStartvignobleBlips(bool)
	if bool and #startvignoble_blips == 0 then
		for station,pos in pairs(startvignoble_locations) do
			local loc = pos
			pos = pos.entering
			...
			table.insert(startvignoble_blips, {blip = blip, pos = loc})
		end
		Citizen.CreateThread(function()
			while #startvignoble_blips > 0 do
				Citizen.Wait(0)
				local inrange = false
				for i,b in ipairs(startvignoble_blips) do
				...
				inrangeofstartvignoble = inrange
				if IsControlJustPressed(1,controllkeyvalid)  and (inrangeofstartvignoble == true) then
					...
					SetVehicleNumberPlateText(spawned_car, "VIGNE")
	...
	elseif bool == false and #startvignoble_blips > 0 then
		for i,b in ipairs(startvignoble_blips) do
