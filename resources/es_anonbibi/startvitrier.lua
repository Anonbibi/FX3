...
local startvitrier_blips ={}
local inrangeofstartvitrier = false
...
--
local blipsnum =  85
local blipstext = 'Dépot des vitrier'
...
AddEventHandler('playerSpawned', function(spawn)
	if firstspawn == 0 then
		ShowStartvitrierBlips(true)
		firstspawn = 1
	end
end)

function IsPlayerInRangeOfStartvitrier()
	return inrangeofstartvitrier
end

function ShowStartvitrierBlips(bool)
	if bool and #startvitrier_blips == 0 then
		for station,pos in pairs(startvitrier_locations) do
			local loc = pos
			pos = pos.entering
			...
			table.insert(startvitrier_blips, {blip = blip, pos = loc})
		end
		Citizen.CreateThread(function()
			while #startvitrier_blips > 0 do
				Citizen.Wait(0)
				local inrange = false
				for i,b in ipairs(startvitrier_blips) do
				...
				inrangeofstartvitrier = inrange
				if IsControlJustPressed(1,controllkeyvalid)  and (inrangeofstartvitrier == true) then
					...
					SetVehicleNumberPlateText(spawned_car, "VITRI")
	...
	elseif bool == false and #startvitrier_blips > 0 then
		for i,b in ipairs(startvitrier_blips) do
