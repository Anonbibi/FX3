...
RegisterNetEvent("police_garages:getVehicles")
RegisterNetEvent('police_garages:SpawnVehicle')
RegisterNetEvent('police_garages:StoreVehicle')
RegisterNetEvent('police_garages:SelVehicle')
...
local garages = {
	{name="Garage", colour=3, id=50, x=462.986, y=-1019.400, z=27.1039},
}
...
function RentrerVehicule()
	TriggerServerEvent('police_garages:CheckForVeh',source)
	CloseMenu()
end
...
function SortirVehicule(vehID)
	local vehID = vehID
	TriggerServerEvent('police_garages:CheckForSpawnVeh', vehID)
	CloseMenu()
end
...
-- 			if IsControlJustPressed(1, 86) then
-- 				TriggerServerEvent('police_garages:CheckForSelVeh',source)
-- 			end
-- 		end
-- 	end
-- end)
...
AddEventHandler("police_garages:getVehicles", function(THEVEHICLES)
    VEHICLES = {}
    VEHICLES = THEVEHICLES
end)

AddEventHandler("playerSpawned", function()
    TriggerServerEvent("police_garages:CheckGarageForVeh")
end)

AddEventHandler('police_garages:SpawnVehicle', function(vehicle, plate, state, primarycolor, secondarycolor, pearlescentcolor, wheelcolor)
	...
	Citizen.CreateThread(function()
		...
		if DoesEntityExist(caisseo) then
			...
			if state == "Sortit" then
				...
				veh = CreateVehicle(car, garageSelected.x, garageSelected.y, garageSelected.z, 270.0, true, false)
				...
				TriggerServerEvent('police_garages:SetVehOut', vehicle, plate)
   				TriggerServerEvent("police_garages:CheckGarageForVeh")
			end
		end
	end)
...
AddEventHandler('police_garages:StoreVehicle', function(vehicle, plate)
	local car = GetHashKey(vehicle)	
	local plate = plate
	Citizen.CreateThread(function()
		Citizen.Wait(3000)
		...
		if DoesEntityExist(caissei) then	
			if plate ~= platecaissei then					
				...
				TriggerServerEvent('police_garages:SetVehIn', plate)
				TriggerServerEvent("police_garages:CheckGarageForVeh")
			end
		else
